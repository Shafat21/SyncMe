	...
	async run(bot, message) {
		...
		if (message.content == `<@!${bot.user.id}>`) {
				...
				.setDescription([
					...
					message.translate('events/message:PREFIX', { PREFIX: settings.prefix, USER: bot.user.username }),
				].join('\n\n'))
				...
				.setFooter(`©  ${bot.user.username} | ${message.author.tag}`, message.member.user.displayAvatarURL({format: 'png'}));
			const row = new MessageActionRow()
				.addComponents(
					new MessageButton()
					.setEmoji('866943415329226782')
		...
		if ([settings.prefix, `<@!${bot.user.id}>`].find(p => message.content.startsWith(p))) {
			...
			if (cmd.conf.ownerOnly && !bot.config.ownerID.includes(message.author.id)) {
				if (message.deletable) message.delete();
				return message.channel.send(`${message.author.tag} Nice Try, Only My Owner can run this command`).then(m => m.timedDelete({ timeout:5000 }));
			}
			...
			if (message.guild) {
				...
				if (neededPermissions.length > 0) {
					bot.logger.error(`Missing permission: \`${neededPermissions.join(', ')}\` in [${message.guild.name}].`);
					if (message.deletable) message.delete();
			...
			// run the command
			bot.commandsUsed++;
			if (bot.config.debug) bot.logger.debug(`Command: ${cmd.help.name} was ran by ${message.author.tag}${!message.guild ? ' in DM\'s' : ` in guild: ${message.guild.name}`}.`);
