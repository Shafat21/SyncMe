// Dependencies
const { Client, Collection } = require('discord.js'), { GuildSchema } = require('../database/models'),
    path = require('path'), { promisify } = require('util'),
...
class Venom extends Client {
    constructor() {
        super({
            ...
            presence: {
                status: 'online',
                activities: [{
                    name: 'SyncMe | -play | -help',
                    type: 'STREAMING',
                    url: 'https://www.twitch.tv/mrwhothebosshere',
                }],
            },
        });
         ...
         * @type {number}
         */
        this.messagesSent = 548516;
        this.commandsUsed = 350651254;
    ...
    SetActivity(type, array = []) {
        ...
        try {
            let j = 0;
            setInterval(() => this.user.setActivity(`${this.Activity[j++ % this.Activity.length]}`, { type: type, url: 'https://www.twitch.tv/mrwhothebosshere' }), 10000);
        } catch (e) {
            console.log(e);
        }
    }
