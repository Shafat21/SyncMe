...
class Speed extends Command {
  ...
  */
	async run(bot, message) {
		...
		if (!player.queue.current.isSeekable) return message.channel.error('musicmisc/speed:LIVESTREAM').then(m => m.timedDelete({ timeout: 10000 }));

		// Make sure Number is a number
		if (isNaN(message.args[0]) || message.args[0] < 0 || message.args[0] > 10) return message.channel.error('musicmisc/speed:INVALID').then(m => m.timedDelete({ timeout: 10000 }));

		// Change speed value
		try {
			player.setSpeed(message.args[0]);
			const msg = await message.channel.send(message.translate('musicmisc/speed:ON_SPD'));
			const embed = new Embed(bot, message.guild)
				.setDescription(message.translate('musicmisc/speed:UPDATED', { NUM: player.speed }));
