...
class Radio extends Command {
  ...
  */
	async run(bot, message, settings) {
		...
		if (!message.member.voice.channel) return message.channel.error('musicmisc/play:NOT_VC').then(m => m.timedDelete({ timeout: 10000 }));
		...
		if (message.member.voice.channel.full && !message.member.voice.channel.permissionsFor(message.guild.me).has('MOVE_MEMBERS')) {
			return message.channel.error('musicmisc/play:VC_FULL').then(m => m.timedDelete({ timeout: 10000 }));
		}
		...
		await getStations({
			...
			if (res.loadType == 'NO_MATCHES') {
				...
				if (!player.queue.current) player.destroy();
				return message.channel.error('musicmisc/play:NO_SONG');
			} else {
	...
	*/
	async callback(bot, interaction, guild, args) {
		...
		// make sure user is in a voice channel
		if (!member.voice.channel) return interaction.reply({ embeds: [channel.error('musicmisc/play:NOT_VC', {}, true)], ephemeral: true });
		...
		if (member.voice.channel.full && !member.voice.channel.permissionsFor(guild.me).has('MOVE_MEMBERS')) {
			return interaction.reply({ embeds: [channel.error('musicmisc/play:VC_FULL', {}, true)], ephemeral: true });
		}
		...
		await getStations({
			...
			if (!data[0]) return interaction.reply({ embeds: [channel.error('musicmisc/radio:MISSING_RADIO', {}, true)], ephemeral: true });
			...
			if (res.loadType == 'NO_MATCHES') {
				// An error occured or couldn't find the track
				if (!player.queue.current) player.destroy();
				return interaction.editReply({ embeds: [channel.error('musicmisc/play:NO_SONG', { }, true)], ephemeral: true });
			} else {
				// add track to queue and play
				if (player.state !== 'CONNECTED') player.connect();
				player.queue.add(res.tracks[0]);
