...
class Clear extends Command {
	...
	*/
	async run(bot, message, settings) {
		...
		if (amount > 30000 && !message.guild.premium) return message.channel.error('moderation/clear:NO_PREM');
			...
			const row = new MessageActionRow()
				.addComponents(
					new MessageButton()
						...
						.setLabel('Confirm')
						.setStyle('SUCCESS')
						.setEmoji(message.channel.checkPerm('USE_EXTERNAL_EMOJIS') ? bot.customEmojis['checkmark'] : '<:icons_Correct:859388130411282442>'),
				)
				.addComponents(
					new MessageButton()
						.setCustomId('cancel')
						.setLabel('Cancel')
						.setStyle('DANGER')
						.setEmoji(message.channel.checkPerm('USE_EXTERNAL_EMOJIS') ? bot.customEmojis['cross'] : '<:icons_Wrong:859388130636988436>'),
				);
	...
	*/
	async callback(bot, interaction, guild, args) {
		...
		// make sure guild is premium if amount > 200
		if (amount > 30000 && !guild.premium) return interaction.reply({ embeds: [channel.error('moderation/clear:NO_PREM', null, true)] });
			...
			const row = new MessageActionRow()
				.addComponents(
					new MessageButton()
						...
						.setLabel('Confirm')
						.setStyle('SUCCESS')
						.setEmoji(channel.checkPerm('USE_EXTERNAL_EMOJIS') ? bot.customEmojis['checkmark'] : '<:icons_Correct:859388130411282442>'),
				)
				.addComponents(
					new MessageButton()
						.setCustomId('cancel')
						.setLabel('Cancel')
						.setStyle('DANGER')
						.setEmoji(channel.checkPerm('USE_EXTERNAL_EMOJIS') ? bot.customEmojis['cross'] : '<:icons_Wrong:859388130636988436>'),
				);
