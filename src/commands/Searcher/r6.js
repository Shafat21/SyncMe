...
class Rainbow6Siege extends Command {
	...
	*/
	async callback(bot, interaction, guild, args) {
		...
		if (resp.color && resp.color == 15158332) {
			...
			interaction.reply({ embeds: [resp], ephemeral: true });
		}
	}
	...
	*/
	async fetchUserData(bot, guild, channel, player, platform, region) {
		...
		.addField(guild.translate('searcher/r6:GENERAL'), guild.translate('searcher/r6:GEN_DATA', { LVL: level, XP: xp.toLocaleString(guild.settings.Language), NAME: current.name, MAX_NAME: max.name, MMR: current.mmr.toLocaleString(guild.settings.Language) }), true)
