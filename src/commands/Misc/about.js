	...
	*/
	createEmbed(bot, guild, settings) {
		return new Embed(bot, guild)
			...
			.setDescription(guild.translate('misc/about:DESC', { URL: bot.config.websiteURL, INVITE: genInviteLink(bot), SERVER: bot.config.SupportServer.link, USERNAME: bot.user.username, PING: bot.ws.ping }))
			.addField(guild.translate('misc/about:MEMBERS'), bot.translate('misc/about:MEMBERS_DESC', { MEMBERS: bot.guilds.cache.reduce((a, g) => a + g.memberCount, 0).toLocaleString(), USERS: bot.users.cache.size.toLocaleString(settings.Language), BOTS: bot.users.cache.filter(user => user.bot).size.toLocaleString(settings.Language) }), true)
			...
			.addField(guild.translate('misc/about:PROCESS'),	bot.translate('misc/about:PROCESS_DESC', { RAM: (process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2) }), true)
