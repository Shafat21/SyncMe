	...
	createEmbed(bot, settings, channel, command, user) {
		if (!command) {
			...
			const embed = new Embed(bot)
				.setAuthor(bot.translate('misc/help:AUTHOR'))
			...
			categories
				...
				.forEach(category => {
					...
					embed.addField(`<:SyncMe:908671315014197278> ${category} [**${length}**]`, `${commands}.`);
					embed.setFooter(`©️ CordDJ x SyncMe | To get info of each command type ${settings.prefix}help [Command] | Have a nice day!`, bot.user.displayAvatarURL({ format: 'png' }) );
				});
			// send message
			return embed;
		} else if (command) {
