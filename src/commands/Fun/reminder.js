	...
	*/
	constructor(bot) {
		super(bot, {
			...
			cooldown: 1000,
			examples: ['reminder 12h Remind me to Vote SyncMe', 'reminder 24h Remind me to use SyncMe'],
		});
	}
		...
		await message.channel.send(message.translate('fun/reminder:MESSAGE', { INFO: message.args.join(' '), TIME: ms(time, { long: true }) })).then(async () => {
			...
			setTimeout(async () => {
				...
				const embed = new Embed(bot, message.guild)
					...
					.setThumbnail('https://cdn.discordapp.com/attachments/833753190859407411/912642589474320384/loader.gif')
