...
class PLoad extends Command {
  ...
  */
	async run(bot, message, settings) {
		...
		if (!message.args[0]) return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('playlistsystem/plload:USAGE')) }).then(m => m.timedDelete({ timeout: 5000 }));
			...
			if (p) {
				...
				content.then(async function() {
					const embed = new Embed(bot, message.guild)
						.setDescription(message.translate('playlistsystem/plload:QUEUE', { NUM: p.songs.length, TITLE: message.args[0] }));
					msg.edit({ embeds: [embed] });
				});
			} else {
				msg.edit(message.translate('playlistsystem/p-load:NO_PLAYLIST'));
			}
		});
	}
}
