...
class PAdd extends Command {
	...
	*/
	constructor(bot) {
		super(bot, {
			...
			cooldown: 3000,
			examples: ['pladd Songs https://www.youtube.com/watch?v=4AnmemzByVY'],
		});
	}
  ...
  */
	async run(bot, message, settings) {
		...
		if (!message.args[0]) return message.channel.error('misc:INCORRECT_FORMAT', { EXAMPLE: settings.prefix.concat(message.translate('playlistsystem/pladd:USAGE')) }).then(m => m.timedDelete({ timeout: 5000 }));
		...
		PlaylistSchema.findOne({
			...
			if (p) {
				...
				try {
					...
					return message.channel.error('playlistsystem/pladd:ERROR', { ERROR: err.message });
				}

				// Workout what to do with the results
				if (res.loadType == 'NO_MATCHES') {
					// An error occured or couldn't find the track
					return message.channel.error('playlistsystem/play:NO_SONG');
					...
					try {
						...
						await p.save();
						message.channel.success('playlistsystem/pladd:SUCCESS', { NUM: newTracks.length, TITLE: message.args[0] });
					} catch (err) {
						if (message.deletable) message.delete();
						bot.logger.error(`Command: '${this.help.name}' has error: ${err.message}.`);
					...
					const embed = new Embed(bot, message.guild)
						.setTitle('playlistsystem/search:TITLE', { TITLE: message.args.join(' ') })
						.setColor("#2f3136")
						.setDescription(message.translate('playlistsystem/search:DESC', { RESULTS: results }));
					...
					if (index < 0 || index > max - 1) return message.reply(message.translate('playlistsystem/search:INVALID', { NUM: max }));

					const track = res.tracks[index];
					p.songs.push(track);
					p.duration = parseInt(p.duration) + parseInt(track.duration);
					await p.save();
					message.channel.success('playlistsystem/pladd:SUCCESS', { NUM: 1, TITLE: track.title });
				} else {
					message.channel.error('playlistsystem/pladd:NO_SONG');
				}
			} else {
				message.channel.error('playlistsystem/pladd:NO_PLAYLIST', { NAME: message.args[0] });
			}
		});
	}
}
