...
class Back extends Command {
  ...
  */
	async run(bot, message) {
		...
		// make sure the number was between 0 and 150
		if (Number(message.args[0]) <= 0 || Number(message.args[0]) > 150) return message.channel.error('music/volume:TOO_HIGH').then(m => m.timedDelete({ timeout: 10000 }));
	...
	*/
	async callback(bot, interaction, guild, args) {
		...
		// make sure volume is between 0 and 150
		if (volume <= 0 || volume > 150) return interaction.reply({ ephemeral: true, embeds: [channel.error('music/volume:TOO_HIGH', { ERROR: null }, true)] });
